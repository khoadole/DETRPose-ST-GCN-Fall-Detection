"""
ST-GCN Models for Action Recognition

Two-Stream Spatial Temporal Graph Convolutional Networks for skeleton-based
action recognition using COCO-17 keypoints.

Reference: https://github.com/yysijie/st-gcn/tree/master/net
"""
...
class GraphConvolution(nn.Module):
    ...
    Args:
        ...
        - t_padding: (int, optional) Temporal zero-padding. Default: 0
    ...
    Shape:
        - Inputs x: Graph sequence in (N, in_channels, T_in, V)
                 A: Graph adjacency matrix in (K, V, V)
        - Output: Graph sequence out in (N, out_channels, T_out, V)
    """
...
class StreamSpatialTemporalGraph(nn.Module):
    ...
    Args:
        ...
        - graph_args: (dict) Args map of Graph Class.
        - num_class: (int) Number of class outputs. If None return pooling features.
        - edge_importance_weighting: (bool) If True, adds learnable importance weights.
        - **kwargs: (optional) Other parameters for graph convolution units.
        
    Shape:
        - Input: (N, in_channels, T_in, V_in)
        - Output: (N, num_class) or (N, out_channels) if num_class is None
    """
...
class TwoStreamSpatialTemporalGraph(nn.Module):
    ...
    Uses two streams:
    - Points stream: (x, y, score) coordinates
    - Motion stream: (dx, dy) frame-to-frame differences
    ...
    Shape:
        - Input: tuple of ((N, 3, T, V), (N, 2, T-1, V))
        - Output: (N, num_class)
    """
...
if __name__ == "__main__":
    print("Testing TwoStreamSpatialTemporalGraph with COCO-17...")
    ...
    batch_size = 4
    T = 30  # Time steps
    V = 17  # Vertices (keypoints)
    
    pts = torch.randn(batch_size, 3, T, V)  # (x, y, score)
    mot = torch.randn(batch_size, 2, T - 1, V)  # (dx, dy)
    
    print(f"\nInput shapes:")
    print(f"  Points: {pts.shape}")
    print(f"  Motion: {mot.shape}")
